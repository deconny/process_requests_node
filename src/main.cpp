...
#include "process_requests_node/RequestProcessor.hpp"

int main(int argc, char *argv[])
{
    rclcpp::init(argc, argv);
    auto node = std::make_shared<rclcpp::Node>("threads_process");

    using AddTwoIntsService = example_interfaces::srv::AddTwoInts;

    auto processor = std::make_shared<RequestProcessor>(4);

    std::string add_service_name_one = "add_two_ints_one";
    processor->add_service<AddTwoIntsService>(add_service_name_one);
    // 创建请求
    auto request_add_one = std::make_shared<AddTwoIntsService::Request>();
    request_add_one->a = 7;
    request_add_one->b = 5;
    processor->process_request<AddTwoIntsService, AddTwoIntsService::Request, AddTwoIntsService::Response>(
    ...
    processor->add_service<AddTwoIntsService>(add_service_name_two);        
    // 处理请求
